...
namespace GitDesktop
{
    internal class ImGuiController : IDisposable
    {
        ...
        private uint vertexArray; // VAO (Vertex Array Object) - description of the vertex data layout
        private uint vertexBuffer; // VBO (Vertex Buffer Object) - buffer for vertex data
        private uint indexBuffer; // EBO (Element Buffer Object) - buffer for index data (which vertices should connect to form triangles)
        ...
        private void OnKeyChar(IKeyboard keyboard, char c) => io.AddInputCharacter(c);

        private void OnKeyUp(IKeyboard keyboard, Key key, int c)
        {
            io.AddKeyEvent(ToImGuiKey(key), false);
            UpdateModifiers();
        }

        private void OnKeyDown(IKeyboard keyboard, Key key, int c)
        {
            io.AddKeyEvent(ToImGuiKey(key), true);
            UpdateModifiers();
        }
        ...
        private void UpdateModifiers()
        {
            ...
            io.AddKeyEvent(
                ImGuiKey.ModAlt,
                keyboard.IsKeyPressed(Key.AltLeft) ||
                keyboard.IsKeyPressed(Key.AltRight));

            io.AddKeyEvent(
                ImGuiKey.ModSuper,
                keyboard.IsKeyPressed(Key.SuperLeft) ||
                keyboard.IsKeyPressed(Key.SuperRight));
        }

        private void OnMouseScroll(IMouse mouse, ScrollWheel wheel) => io.AddMouseWheelEvent(wheel.X, wheel.Y);
        ...
        {
            vertexArray = gl.GenVertexArray(); // generate a new empty OpenGL vertex array and store its ID in _vertexArray
            vertexBuffer = gl.GenBuffer();
            indexBuffer = gl.GenBuffer();

            gl.BindVertexArray(vertexArray); // start working on _vertexArray
            gl.BindBuffer(BufferTargetARB.ArrayBuffer, vertexBuffer); // start working on _vertexBuffer before vertex attributes to tell VBA to use this VBO for vertex attributes
            ...
            gl.EnableVertexAttribArray(0); // by default, all attributes are disabled
            gl.VertexAttribPointer(1, 2, GLEnum.Float, false, 20, (void*)8); // UV attribute (offset is 8 bytes: 2 floats for position)
            gl.EnableVertexAttribArray(1);
            // we interpret the color (one uint (4 bytes)) as 4 unsigned bytes to store each color separately (RGBA)
            gl.VertexAttribPointer(2, 4, GLEnum.UnsignedByte, true, 20, (void*)16); // color attribute
            ...
            gl.BindBuffer(BufferTargetARB.ElementArrayBuffer, indexBuffer); // start working on EBO to tell VAO to use this EBO for index data
        ...
        private unsafe void CreateFontTexture()
        {
            ...
            fontTexture = gl.GenTexture(); // generate a new empty OpenGL texture and store its ID in _fontTexture

            gl.BindTexture(TextureTarget.Texture2D, fontTexture); // now start working on _fontTexture
            ...
            gl.TexParameter(
                        ...
                        (int)GLEnum.Linear);
            gl.BindTexture(TextureTarget.Texture2D, 0); // stop working on _fontTexture

            io.Fonts.SetTexID((nint)fontTexture); // tell ImGui to use the generated OpenGL texture ID for rendering text
            io.Fonts.ClearTexData(); // delete the CPU texture data in ImGui after uploading to GPU by OpenGL
        }

        public void PrepareForRender(double deltaTime)
        {
        ...
        private unsafe void RenderDrawData(ImDrawDataPtr drawData)
        {
            ...
            shader.Use();
            gl.Uniform1(shader.TextureLocation, 0); // use TextureUnit.Texture0
            ...
            {
                gl.UniformMatrix4(
                    shader.ProjectionMatrixLocation,
                    1,
                    false,
                    matrix);
            }            
